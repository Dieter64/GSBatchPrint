...
win_pr2_put_params(gx_device * pdev, gs_param_list * plist)
{
    ...
    int  old_input_bin = wdev->input_bin;
...
win_pr2_getdc(gx_device_win_pr2 * wdev)
{
    ...
    int i, n;
    POINT *pp;
    int paperindex, usercopies;
    int paperwidth, paperheight;
    int orientation, inputbin;
    int papersize;
    char papername[64];
    ...
    if (wdev->input_bin){
      inputbin = wdev->input_bin;
    }

    if (wdev->user_copies > 1) {
        usercopies = wdev->user_copies;
    }
    
    pidevmode->dmFields &= ~(DM_PAPERSIZE | DM_ORIENTATION | DM_COLOR | DM_PAPERLENGTH | DM_PAPERWIDTH | DM_DUPLEX | DM_COPIES);
    ...
    if (inputbin){
        pidevmode->dmDefaultSource = inputbin;
    }
    if (usercopies > 1) {
        pidevmode->dmCopies = usercopies;
    }
...
#else
    wdev->hdcprn = CreateDC(NULL, device, NULL, podevmode);
#endif
...
win_pr2_update_dev(gx_device_win_pr2 * dev, LPDEVMODE pdevmode)
{
    ...
    if (pdevmode->dmFields & DM_DEFAULTSOURCE){
        dev->input_bin = pdevmode->dmDefaultSource;
    }
    if (pdevmode->dmFields & DM_COPIES){
        dev->user_copies = pdevmode->dmCopies;
    }
...
win_pr2_update_win(gx_device_win_pr2 * dev, LPDEVMODE pdevmode)
{
    ...
    if (dev->input_bin) {
        ...
        pdevmode->dmDefaultSource = dev->input_bin;
    }
    if (dev->user_copies > 1) {
        pdevmode->dmFields |= DM_COPIES;
        pdevmode->dmDefaultSource = dev->user_copies;
    }
    return 0;
}
        ...
        case 0:
            {
                ...
                param_read_int(dict.list, "InputBin", &wdev->input_bin);
                ...
                if (wdev->win32_hdevmode) {
                    ...
                    if (devmode) {
                        ...
                        devmode->dmDefaultSource = wdev->input_bin;
                        devmode->dmOrientation = wdev->user_orient;
                        devmode->dmColor = wdev->user_color;
                        devmode->dmFields |= (DM_PAPERSIZE | DM_ORIENTATION | DM_PAPERSIZE | DM_COPIES | DM_COLOR | DM_DEFAULTSOURCE);
                        GlobalUnlock(wdev->win32_hdevmode);
                    }
                }
            }
...
win_pr2_write_user_settings(gx_device_win_pr2 * wdev, gs_param_list * plist)
{
    ...
    /* new DR input bin */  
    code = param_write_int(dict.list, "InputBin", &wdev->input_bin);
    if (code < 0) goto error;
...
win_pr2_print_setup_interaction(gx_device_win_pr2 * wdev, int mode)
{
    ...
    wdev->input_bin = devmode->dmDefaultSource;
